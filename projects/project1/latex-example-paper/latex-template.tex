...
This project gives the methodology in details of what we have done to compute a 
prediction model for the Higgs Boson's challenge in Kaggle, provided by CERN data
for a practical and scientific use.
In order to compute this model we had a limitation in the methods we could use, 
that is only the simple models that we already saw in class including 
the gradient descent, the least squares method and the linear regression.

So the challenge was to find a pretty good estimation using only a few models 
that certainly can't represent perfectly this data set.

\section{Methodology}
\label{sec:structure-paper}

We followed a long path in order to improve our result, from random inneficient
prediction to polynomial basis model constructed and regression used to find a 
weights vector more representative. 

Our implementation took some preparation steps that we will present below.

\subsection{Pre-computational analysis}
As the title suggest, before even trying a model we decided to analyse the
data entries in order to possibly find some correlations between the features 
and/or the supposed outputs. 

Our results were not decisive at all, we didn't manage to find something 
interesting noting that we couldn't use the python libraries at all to compute 
some data analysis so we was limited in this by the contest reglementation and 
the time we had. 
So for the features we finally choose to keep them all and treat the data all as a one, 
without needing a preselection of interesting data. 

Then we managed the missing values (-999) before the standardization by 
setting each of them to the mean value in order to avoid a bad contribution 
for the model computing phase.

\subsection{Models comparisons}
Then we tried to use multiple of our models to see what was the outcoming results
and compare them one to another to have a first idea of what kind of data we are facing.
So we trained on the prepared dataset with least squares, gradient descent, 
stochastic gradient descent, and the regression models we had implemented.

What we came to was surprising, the logistic regressions models were not converging,
and we the best results we seems to have was with the least squares model.
